...
def get_csd_reader():
    ...
    Set the CSD_DATABASE_PATH environment variable to your local CSD ``.sqlite``
    database. If unset, falls back to the CSD Python API's default resolution.
    """
    csd_db_path = os.environ.get('CSD_DATABASE_PATH', '')
    if csd_db_path:
        return io.EntryReader(csd_db_path)
    return io.EntryReader('CSD')
...
def get_refcode_families(input_parameters):
    ...
    filename = 'csd_db_analysis_large/db_data/' + input_parameters['data_prefix'] + '_csd_refcode_families.json'
...
def cluster_refcode_families(input_parameters):  
    ...
    refcode_families_f = 'csd_db_analysis_large/db_data/' + input_parameters['data_prefix'] + '_csd_refcode_families.json'
    if not os.path.exists(refcode_families_f):
        ...
        csd_entries = get_csd_reader()
        families_clustered = {}
        for family in tqdm(refcode_families):
    ...
    filename = 'csd_db_analysis_large/db_data/' + input_parameters['data_prefix'] + '_csd_refcode_families_clustered.json'
...
def save_cifs_and_pdbs(input_parameters):
    """
    For each structure in <data_prefix>_csd_refcode_families_clustered.json,
    this function saves both:
      - An mmCIF in '.../<refcode>.mmcif'
      Additionally, it attempts to retrieve the SMILES of the structure and
      embeds it in the mmCIF file as `_chemical_identifier_smiles`.
    """
    import os
    import json
    from ccdc import io

    # 0. Get structures to necessarily include
    structures_to_include = input_parameters.get("structures_to_include", [])

    # 1. Paths to input and output folders
    families_file = (
        'csd_db_analysis_large/db_data/' 
        + input_parameters['data_prefix'] 
        + '_csd_refcode_families_clustered.json'
    )
    cif_output_folder = (
        'csd_db_analysis_large/db_data/' + input_parameters['data_prefix'] + '_cifs/'
    )
    os.makedirs(cif_output_folder, exist_ok=True)

    # Track refcodes already written to disk so reruns can resume
    existing_cifs = [
        f for f in os.listdir(cif_output_folder) if f.lower().endswith(".mmcif")
    ]
    processed_refcodes = set(os.path.splitext(f)[0] for f in existing_cifs)
    if processed_refcodes:
        print(
            f"[INFO] Skipping {len(processed_refcodes)} mmCIFs already present in "
            f"{cif_output_folder}"
        )

    # 2. Check the JSON file with clustered families
    if not os.path.exists(families_file):
        print(f"File not found: {families_file}")
        return

    # 3. Load the dictionary of clustered families
    with open(families_file, 'r') as f:
        families_clustered = json.load(f)

    # 4. Initialize the CSD entry reader
    csd_reader = get_csd_reader()

    # 5. Loop over each family and each cluster
    for family, clusters in families_clustered.items():
        for cluster in clusters:
            for refcode in cluster:
                if refcode in processed_refcodes: # Skip if already processed (e.g. from structures_to_include)
                    continue
                try:
                    # a) Retrieve the CSD entry and crystal
                    entry = csd_reader.entry(refcode)
                    crystal = entry.crystal

                    # b) Prepare an mmCIF filename
                    cif_path = os.path.join(cif_output_folder, f"{refcode}.mmcif")

                    # c) Use a CrystalWriter to save the crystal as mmCIF
                    with io.CrystalWriter(cif_path) as cw:
                        cw.write(crystal)
                    print(f"[CIF] Saved {refcode} -> {cif_path}")
                    processed_refcodes.add(refcode)

                    # d) Attempt to retrieve a SMILES string and embed it in the mmCIF file
                    try:
                        smiles_str = entry.molecule.smiles
                        if smiles_str:
                            # Insert a custom tag, for example '_chemical_identifier_smiles'
                            # We do this by reading the mmCIF file lines,
                            # inserting the tag after 'data_...' line, and writing it back.
                            with open(cif_path, 'r') as f_in:
                                lines = f_in.readlines()

                            # Check if SMILES already exists to avoid duplicates
                            has_smiles = any('_chemical_identifier_smiles' in line for line in lines)
                            
                            if not has_smiles:
                                inserted = False
                                for idx, line in enumerate(lines):
                                    if line.lower().startswith('data_'):
                                        # Insert right after the data_ line (standard location for custom tags)
                                        lines.insert(
                                            idx + 1,
                                            f"_chemical_identifier_smiles '{smiles_str}'\n"
                                        )
                                        inserted = True
                                        break
                                    
                                if not inserted:
                                    # Fallback: append at the end if we couldn't find data_ line
                                    lines.append(
                                        f"_chemical_identifier_smiles '{smiles_str}'\n"
                                    )

                                with open(cif_path, 'w') as f_out:
                                    f_out.writelines(lines)
                                print(f"[CIF] Embedded SMILES into {refcode}.mmcif: {smiles_str}")
                            else:
                                print(f"[CIF] SMILES already exists in {refcode}.mmcif, skipping insertion")
                    
                    except Exception as e_smiles:
                        print(f"Could not embed SMILES for {refcode} due to: {e_smiles}")

                except Exception as e:
                    print(f"Could not save files for {refcode} due to error: {e}")

    # 6. Process and save structures from the 'structures_to_include' list
    if structures_to_include:
        print("\nProcessing specifically requested structures from 'structures_to_include':")
        for refcode in structures_to_include:
            if refcode in processed_refcodes:
                print(f"[INFO] {refcode} was already processed from clustered families.")
                continue
            try:
                # a) Retrieve the CSD entry and crystal
                entry = csd_reader.entry(refcode)
                ...
                try:
                    ...
                    print(f"Could not embed SMILES for {refcode} (from structures_to_include) due to: {e_smiles}")

            except ccdc.entry.InvalidEntryError:
                print(f"[ERROR] Refcode {refcode} (from structures_to_include) not found in CSD.")
            except Exception as e:
                print(f"Could not save files for {refcode} (from structures_to_include) due to error: {e}")
...
def get_unique_structures(input_parameters):
    ...
    refcode_families_clusters_f  = '../csd_db_analysis_large/db_data/' + input_parameters['data_prefix'] + '_csd_refcode_families_clustered.json'
    ...
    # Specify the filename for the clustered families
    filename = 'csd_db_analysis_large/db_data/' + input_parameters['data_prefix'] + '_csd_refcode_families_unique_structures.json'
